...
module.exports = {
  conf: {
    aliases: ["puan"],
    name: "puan",
    help: "puan [ekle/sil/gönder] [kullanıcı] [sayı]"
  },
  ...
  run: async (client, message, args, embed, prefix) => {
    ...
    if (args[0] === "ekle" || args[0] === "add") {
      ...
      if (coinData && client.ranks.some(x => coinData.coin >= x.coin && !member.roles.cache.has(x.role))) {
        ...
        message.guild.channels.cache.get(conf.rankLog).send(embed.setDescription(`${member.toString()} üyesine ${message.member.toString()} tarafından **${count}** adet puan eklendi ve kişiye ${roles.filter(x => roles.indexOf(x) === roles.length-1).map(x => `<@&${x.role}>`).join("\n")} rolleri verildi!`));
      }
      ...
      if (coinData && client.ranks.some(x => coinData.coin < x.coin && member.roles.cache.has(x.role))) {
        ...
        embed.setColor("RED");
        message.guild.channels.cache.get(conf.rankLog).send(embed.setDescription(`${member.toString()} üyesinden ${message.member.toString()} tarafından **${count}** adet puan çıkarıldı ve kişiden ${roles.map(x => `<@&${x.role}>`).join(", ")} rolleri alındı!`));
      }
      message.channel.send(embed.setDescription(`Başarıyla ${member.toString()} kullanıcısından **${count}** adet puan çıkarıldı! \n\n${removedRoles.length > 0 ? `Alınan roller: \n${removedRoles.map(x => `<@&${x.role}>`).join("\n")}` : ""}`));
      ...
      if (!coinData || coinData && count > coinData.coin) return message.channel.send(embed.setDescription("Göndereceğin sayı kendi puanından yüksek olamaz!"));
      ...
      message.channel.send(embed.setDescription(`${member.toString()} kişisine başarıyla **${count}** puan gönderildi!`));
    }
  }
};

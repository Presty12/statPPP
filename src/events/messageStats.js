  ...
  if (conf.staffs.some(x => message.member.roles.cache.has(x))) {
    ...
    if (num && (num % conf.messageCount) === 0) {
      ...
      if (coinData && client.ranks.some(x => coinData.coin === x.coin)) {
        ...
        message.guild.channels.cache.get(conf.rankLog).send(embed.setDescription(`${message.member.toString()} üyesi **${coinData.coin}** puan hedefine ulaştı ve ${Array.isArray(newRank.role) ? newRank.role.map(x => `<@&${x}>`).join(", ") : `<@&${newRank.role}>`} rolü verildi!`));
      }
